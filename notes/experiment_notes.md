...
- The pipeline now supports any input CSV with a regular timestamp grid. Cadence is inferred from the `timestamp` column and stored in the experiment manifest, so the same `src/run_experiment.py` entrypoint works for 15-minute, 10-minute, or other regular cadences.
...
- Native-cadence TIM Milan benchmark flow:
  1. `python data/build_tim_milan_dataset.py`
  2. `python src/run_experiment.py --data-path data/data_tim_milan_10min.csv --context-length 48 --horizon 48 --models lstm,deepar,chronos2`
- Trainable TIM Milan subset flow:
  1. `python data/build_tim_milan_dataset.py`
  2. `python data/build_tim_milan_trainable_subset.py`
  3. `python src/run_experiment.py --data-path data/data_tim_milan_10min_trainable_200.csv --context-length 48 --horizon 48 --models lstm,deepar,chronos2`
...
## Data README
### TIM Milan public benchmark
- We added a public telecom benchmark builder at `data/build_tim_milan_dataset.py`.
- The raw source is the open **TIM Milan internet-activity dataset** mirrored by **O-RAN SC** for AI/ML and energy-savings experimentation.
- The script downloads the daily parsed archives from the O-RAN dataset mirror into `data/raw/tim_milan/`.
- Each raw daily archive contains long-format records with:
  - a Milan grid cell identifier
  - a timestamp
  - an `InternetTraffic` value
- In this repo, we use only the `InternetTraffic` field from the TIM Milan dataset.
- These values are anonymized and normalized internet-activity counts. They are useful for traffic-pattern forecasting and network-operation experiments, but they are **not literal Mbps measurements**.

### What the harness does
- It reads all selected daily archives and concatenates them into one long table.
- It preserves the **native 10-minute cadence** of the original TIM dataset. We do not resample it to 15 minutes.
- It converts the long table into a wide time-series CSV with:
  - `timestamp` as the first column
  - one numeric series column per Milan grid cell, named like `MI4259`
- It infers the regular cadence from the timestamps and aligns the full city-wide matrix on that grid.
- It keeps **all fully observed cells** over the full aligned horizon and drops only incomplete cells.
- This yields the clean complete-cell intersection of the Milan dataset, without imputation and without hand-picked subset selection.
- Finally it writes:
...
### Trainable subset
- We also provide a second script at `data/build_tim_milan_trainable_subset.py` for lighter trainable runs.
- It reads the cleaned full TIM Milan benchmark and selects `200` complete cells with a neutral deterministic rule:
  - sort complete cells by ascending `square_id`
  - keep the first `200`
- This avoids activity-based or variance-based cell cherry-picking.
- It writes:
...
- Build the lighter trainable subset:
  `python data/build_tim_milan_trainable_subset.py`
- Run the forecasting pipeline on the generated native-cadence dataset:
  `python src/run_experiment.py --data-path data/data_tim_milan_10min.csv --context-length 48 --horizon 48 --models lstm,deepar,chronos2`
- Or run on the smaller trainable subset:
...
- The pipeline is cadence-flexible, so the same runner also works on the existing 15-minute dataset `data/data_1to672.csv`.
...
- In practice, zero-shot forecasting is operationally attractive because it can reduce or avoid recurring retraining cycles when the network undergoes macro distribution shifts across years, seasons, or large-scale usage changes.
- The paper-level tradeoff we want to highlight is: trainable baselines can be stronger when retrained on-domain, while zero-shot foundation models can be more practical under long-term distribution drift because deployment does not depend on repeated yearly re-training.
